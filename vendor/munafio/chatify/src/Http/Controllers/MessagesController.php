...
class MessagesController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        return view('Chatify::pages.app', compact('data', 'userInfo', 'title', 'id', 'messengerColor', 'dark_mode'));
    }


    public function indexId($id = null)
    {
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch ?? null,
            'userInfo' => \CustomHelpers::getUserInformation($request['id']),
            'person' => \CustomHelpers::getUser($request['id']),
    ...
    public function getContacts(Request $request)
{
    ...
    if (count($usersList) > 0) {
        $contacts = []; // Initialize an empty array
        foreach ($usersList as $user) {
            $contacts[] = Chatify::getContactItem($user); // Append to the array
        }
    } else {
    ...
    public function search(Request $request)
{
    ...
    $userId = Auth::user()->id; // Current user's ID
    
    // Retrieve user records with search input and pagination
    $records = User::where('id', '!=', $userId)
        ->where(function ($query) use ($input, $userId) {
            $query->where('first_name', 'LIKE', "%{$input}%")
                  ->orWhere('last_name', 'LIKE', "%{$input}%")
                  ->orWhere('email', 'LIKE', "%{$input}%");
        })
        ->whereHas('userRelationships', function ($query) use ($userId) {
            $query->where('relative_id', $userId)
    ...
    foreach ($records->items() as $record) {
        // Render view for each user record
