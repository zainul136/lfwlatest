    ...
    public function getContacts()
{
    ...
    $contacts = User::join('user_relationships', function ($join) use ($loggedInUserId) {
        $join->on('users.id', '=', 'user_relationships.relative_id')
...
public function isInRelationshipWithLoggedInUser()
{
...
public function relationships()
{
    ...
    public function search(Request $request)
    {
        ...
        // Find the IDs of users in a direct relationship with the current user
        $relationshipIds = DB::table('user_relationships')
            ->where('user_id', $loggedInUserId)
            ->pluck('relative_id');
    
        // Search for users based on first name and filter by relationship
        ...
        foreach ($users->items() as $index => $user) {
            $users[$index] += Chatify::getUserWithAvatar($user);
        }
