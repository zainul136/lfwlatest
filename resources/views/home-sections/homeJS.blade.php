...
<script>
    ...
    indexValue[1]
    showImg(indexValue, 3, 23123, );

    function btm_slide(e, imgcount, rand, indexV) {
        for (let index = 0; index < imgcarousel.length; index++) {
            const element = imgcarousel[index];
            if (document.querySelector(`#${element.id} .btmslidesunique${rand}`)) {

                showImg(indexValue[index] = e, imgcount, rand);
            }
        }
    }

    function side_slide(e, imgcount, rand) {
        for (let index = 0; index < imgcarousel.length; index++) {
            const element = imgcarousel[index];
            if (document.querySelector(`#${element.id} .btmslidesunique${rand}`)) {
    ...
    function showImg(e, imgcount, rand) {
        for (let index = 0; index < imgcarousel.length; index++) {
            const element = imgcarousel[index];
            if (document.querySelector(`#${element.id} .btmslidesunique${rand}`)) {
                console.log(document.querySelector(`#${element.id} .btmslidesunique${rand}`))
                console.log(index)
                var i;
                // console.log(rand, imgcount)
                const imgpub = document.querySelectorAll(`.card2contentunique${rand} .card2-top .images img`);
                const sliderpub = document.querySelectorAll(`.btmslidesunique${rand} span`);
                if (e > imgcount) {
                    indexValue[index] = 1
                }
                if (e < 1) {
                    indexValue[index] = imgcount
                }
                for (i = 0; i < imgcount; i++) {
                    imgpub[i].style.display = "none";
                }
                for (i = 0; i < sliderpub.length; i++) {
                    sliderpub[i].style.background = "rgba(255,255,255,0.1)";
                }
                imgpub[indexValue[index] - 1].style.display = "block";


                sliderpub[indexValue[index] - 1].style.background = "white";
            }
        }
    }
...
<!-- Uplaod Image  -->
<script>
    const imageBtn = document.getElementById("UploadImages")
    ...
    const PostDD = (value) => {
        if (value == "privatePeopleSearch") {
            document.getElementById("privatePeopleSearchName").style.display = "flex";
            document.getElementById("privatePeopleSearchHere").style.display = "flex";
            document.getElementById("labelForPrivateSearch").style.display = "flex";
            ...
            document.getElementById("privatePeopleSearchName").style.display = "none";
            document.getElementById("privatePeopleSearchHere").style.display = "none";
            document.getElementById("labelForPrivateSearch").style.display = "none";
        }
    }
    // PUBLIC OR SPECIFI POST DROPDOWN
    ...
    const InputTags = (e) => {
        ...
        docBtn.addEventListener("change", (e) => {
            ...
            for (let index = 0; index < files.length; index++) {
                ...
                div.innerHTML =
                    `<div> <img src=${documentExtImg}> ${fileName} </div> <span class="material-symbols-outlined me" onclick="deleteDoc('${fileName + random}')">cancel</span>`
                docs.append(div)
                // //console.log(div)
            }
        ...
        if (toggle) {
            ...
            if (confirm("Are you sure you want to close? All Unsaved Changes will Be Lost")) {
                ...
                for (let index = 0; index < tagdivs.length; index++) {
                    ...
                    div.id = element.parentElement.id
                    div.append(element)
                    //console.log(element)
                    // div.innerHTML = element
                    document.querySelector("#tagsrow").append(div)
                }
...
<div class="post-section" id="postId1">
  ...
  `
    ...
    function uploadVideoFunc(imageBtn) {
        ...
        reader.onloadend = function() {
            ...
            videoInput.value = JSON.stringify({
                base64,
                filename: file.name
            });
...
<script>
    ...
    const OpenImageModal = (e, value, src, imgsContainer) => {
        ...
        if (value) {
            ...
            ModalImg.src = src
            navButtons[0].setAttribute("onclick", `ModalImageSlider(-1, '${imgsContainer}')`)
            navButtons[1].setAttribute("onclick", `ModalImageSlider(1, '${imgsContainer}')`)
        } else {
            if (e.target == ModalImg || e.target.classList.contains("navButtons")) return
            modalImageIndex = 0
            Modal.style.display = "none"
        }
    }
    const ModalImageSlider = (e, container) => {
        const ModalImg = document.querySelector(".ModalImg")
        const images = document.querySelectorAll(`.${container} .images img`)
        modalImageIndex += e
        if (modalImageIndex > images.length - 1) {
            modalImageIndex = 0
        } else if (modalImageIndex == -1) {
...
<script>
    ...
    docBtn.addEventListener("change", (e) => {
        ...
        for (let index = 0; index < files.length; index++) {
            ...
            // div.setAttribute("onclick", "deleteDoc(event)")
            /* div.innerHTML = `<div> <img src=${documentExtImg}> ${fileName.substr(0, 10)} </div> <span class="material-symbols-outlined" onclick="deleteX('${fileName + random}')">cancel</span>`;
            docs.append(div) */
            div.innerHTML =
                `<div> <img src=${documentExtImg}> ${fileName} </div> <span class="material-symbols-outlined" onclick="deleteX('${fileName + random}')">cancel</span>`
            docs.append(div)
