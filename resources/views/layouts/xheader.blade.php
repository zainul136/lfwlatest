        ...
        <div class="container-fluid header">
                        ...
                        <form class="d-flex d-none d-lg-block searchBar" action="{{ route('search') }}" method="get"
                            ...
                            <div class="d-flex position-relative w-100">
                                ...
                                <button class="position-absolute"
                                        style="top: 20%; right: 5%; background: none; border: none;" type="submit">
                        ...
                        <div class=" navbar" id="navbarSupportedContent">
                            <ul class="navbar-nav ms-auto mb-lg-0">
                                ...
                                @php
                                    ...
                                    $users = \App\Models\UserRelationship::query()->where('relative_id', $loggedInUser->id)
                                        ->where('status', 'pending')
                                        ->with(['relative', 'relationship:id,type'])
                                        ->get();
                                    $privatePosts = \App\Models\PrivateTaggedPeople::query()->where('user_id', $userId)->where('view_post','=','0')->with('post')->get();
                                         $postCountAfterDeathConfirmation = 0;
                                            foreach($privatePosts as $privatePost)

                                         $checkDeathConfirmationUsers = \App\Models\DeathConfirmation::query()->where('user_id','=', $privatePost->post->user_id)
                                            ->where('is_alive', 0)
                                            ->where('confirmation_status', 1)->first();

                                @endphp
                                <li class="nav-item dropdown">
                                    <a class="nav-link text-center" role="button" id="notification-menu" data-bs-toggle="dropdown" aria-expanded="false">
                                     <span style=";">
                                <i class="fas fa-bell fa-2x" style="color: #99ddcc;"></i>
                                      @php
                                  $deathCount = $deathNotifications->count();

                     $privatePostCount =  $privatePosts->count();
                    $count = $deathCount + $privatePostCount;
                 if ($count > 0) {
                     $notificationCountClass = '';
                     if ($count > 9 && $count <= 99) {
                          echo '<span style="position: absolute; top: 3px; right: 0; color: white; background-color: #99ddcc; border-radius: 50%; padding-left: 3px; padding-right: 3px; border: 2px solid #99ddcc; font-size: 12px; margin-right: 15px;">'.$count.'</span>';
                     } elseif ($count > 99) {
                        echo '<span style="position: absolute; top: 3px; right: 0; color: white; background-color: #99ddcc; border-radius: 50%; padding-left: 3px; padding-right: 3px; border: 2px solid #99ddcc; font-size: 12px; margin-right: 6px;">'.$count.'</span>';
                     }

                        echo '<span style="position: absolute; top: 3px; right: 0; color: white; background-color: #99ddcc; border-radius: 50%; padding-left: 5px; padding-right: 5px; border: 2px solid #99ddcc; font-size: 12px; margin-right: 16px;">' . $count. '</span>';                }
                      @endphp
            ...
            @if ($count > 0)
                <span class="badge badge-danger" id="notification-badge"></span>
                                    ...
                                    <ul class="dropdown-menu NotificationDropdown pl-2 pr-0 pt-0 pb-4">
                                        <div class="requests">
                                            @foreach($deathNotifications as $dNotification)
                                                ...
                                                <div class="mb-5">
                                                    ...
                                                    <p>We regret to inform you that {{ $informUser->first_name }} {{ $informUser->last_name }} has notified us of the passing of {{ $deathUsers->first_name }} ({{ $deathUsers->email ?? '' }}).</p>
                                                    <div class="d-flex">
                                            ...
                                            @foreach($privatePosts as $privatePost)
                                                ...
                                                @foreach ($checkDeathConfirmationUsers as $checkDeath)
                                                    @php
                                                        $deathUsers = \App\Models\User::query()->find($checkDeath->user_id);
                                                        $privatePostCreate =  \App\Models\Post::query()->where('id',$privatePost->post_id)->first();
                                                    @endphp
                                                    <div class="mb-5">

                                                        <p>We extend our heartfelt condolences on the loss of your beloved {{ $deathUsers->first_name  ??''}}. Upon clicking the "View Post" button, created on this  <span><strong>{{ $privatePostCreate->created_at->format('d F Y') }} {{ $privatePostCreate->created_at->format('h:i A') }}</strong></span>
                                                            you can access the private Posts,  type of post is <span><strong class="font-weight-bold" style="font-weight: 900">{{$privatePost->post->post_type}}</strong></span></p>
                                                        <div class="d-flex">
                            ...
                            <li class="nav-item dropdown">
                                <a class="nav-link" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    <div class="d-flex align-items-center">
                                        <img src="{{ isset($data['user_information']->profile_picture) ? asset('user_media/' . auth()->user()->id . '/profile_picture/' . auth()->user()->userInformation->profile_picture) : asset('storage/assets/images/profile.jpeg') }}" class="girl postProfileIcon" />

                                        <p class="emile">{{ auth()->user()->first_name ?? '' }}</p>
                                        <img src="{{ asset('storage/assets/images/arrow.png') }}" class="arrow" />
                                    </div>
                                </a>
