...
@section('content')
    <div class="container-fluid">
        ...
        <div class="form-head d-flex align-items-center flex-wrap justify-content-between mb-sm-5 mb-3">
            <h2 class="font-w600 mb-0 text-black">Staff Information</h2>
        </div>
        <div class="row">
            <div class="col-xl-12">
                <div class="table-responsive table-hover fs-14 bg-white">
                    ...
                    <table class="table display mb-4 dataTablesCard short-one card-table text-black" id="example6">
                        ...
                        @foreach($staffs as $item)
                                ...
                                <?php
                                $userProfileImages = \App\Models\UserInformation::query()->where('user_id','=',$item->user->id)->first();

                                $profilePicture = isset($userProfileImages->profile_picture) ?
                                    asset("user_media/{$item->user_id}/profile_picture/{$userProfileImages->profile_picture}") :
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center" style="gap: 8px;">
                                        ...
                                        <div>
                                            <p class="mb-0 font-w500">{{ $item->user->first_name}}</p>
                                            <p class="mb-0" style="font-size: 14px;">{{$item->user->email}}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>{{$item->user->user_information ? $item->user->user_information->gender : 'Not Yet Set User info '}}</td>
                                <td>{{$item->user->user_information ? $item->user->user_information->date_of_birth : 'Not Yet Set User info '}}</td>
                                <td>{{$item->user->user_information ? $item->user->user_information->phone_number : 'Not Yet Set User info '}}</td>
                                <td>{{$item->user->user_information ? $item->user->user_information->city : 'Not Yet Set User info '}}</td>
                                <td>{{$item->user->user_information ? $item->user->user_information->country : 'Not Yet Set User info '}}</td>
                                <td>
                                    @if($item->user->active_status == '0')
                                        <div class="active-decease-status status-active">Active</div>
                                    @elseif($item->user->active_status == '1')
                                ...
                                <div id="user-info-edit-modal-{{ $item->id }}" class="modal-box user-info-edit-modal "   >
                                    <form action="{{route('update-staff-record')}}" method="post">
                                        <input type="text" name="user_id" value="{{$item->user->id}}" hidden="">
                                        @csrf
                                    <!-- Modal content -->
                                    <div class="modal-content">
                                        <div class="modal-content-first mt-6">
                                            <p class="main-black" style="font-size: 18px;">Edit Staff Info</p>
                                        ...
                                        <div class="mt-4 px-3">
                                            ...
                                            <div class="d-flex align-items-center" style="gap: 16px;">
                                                <input type="text" name="first_name" class="w-100" id="" value="{{ $item->user->first_name  ??''}}" style="border:1px solid rgba(153, 153, 153, 0.7); border-radius: 15px; padding: 12px 20px; font-size: 14px;">
                                            ...
                                            <div class="d-flex align-items-center" style="gap: 16px;">
                                                <input type="text" name="last_name" class="w-100" id="" value="{{ $item->user->last_name  ??''}}" style="border:1px solid rgba(153, 153, 153, 0.7); border-radius: 15px; padding: 12px 20px; font-size: 14px;">
                                            ...
                                            <div class="d-flex align-items-center" style="gap: 16px;">
                                                <input type="email" name="email" class="w-100" id="" value="{{ $item->user->email ?? '' }}" style="border:1px solid rgba(153, 153, 153, 0.7); border-radius: 15px; padding: 12px 20px; font-size: 14px;">
                                                <svg class="edit-last-name-btn" width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M9.25 2.5L11.5 4.75M7.75 13H13.75M1.75 10L1 13L4 12.25L12.6895 3.5605C12.9707 3.27921 13.1287 2.89775 13.1287 2.5C13.1287 2.10226 12.9707 1.72079 12.6895 1.4395L12.5605 1.3105C12.2792 1.0293 11.8977 0.871323 11.5 0.871323C11.1023 0.871323 10.7208 1.0293 10.4395 1.3105L1.75 10Z" stroke="#99DDCC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            </div>
                                            ...
                                            <div class="d-flex align-items-center" style="gap: 16px;">
                                                <select class="form-select w-100 bg-white" id="country" name="country" style="border:1px solid rgba(153, 153, 153, 0.7); border-radius: 15px; padding: 12px 20px; font-size: 14px;">
                                                    ...
                                                    @foreach($countries = \App\Models\Country::all() as $country)
                                                        @php
                                                            $selected = optional($item->user->userInformation)->country === $country->code ? 'selected' : '';
                                                        @endphp
                                            ...
                                            <div class="d-flex align-items-center" style="gap: 16px;">
                                                <input type="date"  name="date_of_birth" value="{{ $item->user->userInformation->date_of_birth ??'' }}" class="w-100" id="" style="border:1px solid rgba(153, 153, 153, 0.7); border-radius: 15px; padding: 12px 20px; font-size: 14px;">
                                                <svg class="edit-last-name-btn" width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M9.25 2.5L11.5 4.75M7.75 13H13.75M1.75 10L1 13L4 12.25L12.6895 3.5605C12.9707 3.27921 13.1287 2.89775 13.1287 2.5C13.1287 2.10226 12.9707 1.72079 12.6895 1.4395L12.5605 1.3105C12.2792 1.0293 11.8977 0.871323 11.5 0.871323C11.1023 0.871323 10.7208 1.0293 10.4395 1.3105L1.75 10Z" stroke="#99DDCC" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            </div>
                                ...
                                <div id="user-info-modal-{{ $item->id }}" class="modal-box">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-content-first">
                                            <p>Are You Sure You Want To Delete <strong>{{ $item->user->first_name }} {{ $item->user->last_name }}</strong>?</p>
                                            <!-- <span class="close">&times;</span> -->
                                        </div>
                                        <div class="modal-cancwl-del-container">
                                            <button class="modal-cancel-btn close-delete-button">Cancel</button>
                                            <form action="{{ route('delete-staff', ['user' => $item->user->id]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
    ...
    <script>
        ...
        document.addEventListener('click', function(event) {
            if (event.target.classList.contains('close-edit-button')) {
             ...
             document.addEventListener('click', function(event) {
            if (event.target.classList.contains('close-delete-button')) {
                event.target.parentElement.parentElement.parentElement.style.display = 'none';
            }
        });
