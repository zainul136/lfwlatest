  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        const profilePicturePath = new URL(`user_media/${data.fetch.id}/profile_picture/${data.userInfo.profile_picture}`, window.location.origin).href;

// avatar photo
        $(".messenger-infoView .avatar").css("background-image", `url("${profilePicturePath}")`);
        $(".header-avatar").css("background-image", `url("${profilePicturePath}")`);
  ...
  if (!contactsLoading && !noMoreContacts) {
      ...
      $.ajax({
          ...
          success: (data) => {
              ...
              $(".listOfContacts").empty();
              contacts.forEach((contact) => {
                  $(".listOfContacts").append(contact);
              });

              // Update other UI elements and pagination logic
...
document.addEventListener('click', function(event) {
  if (event.target.matches('.messenger-list-item')) {
      var element = event.target;
      var dataContact = element.getAttribute('data-contact');
